...
function LihatApprovmentIa01({ onBack, onNavigate }) {
  const id = useParams();

  // Fetch IA-01 submissions
  const { data: ia01, isLoading, isError, error } = useQuery({
    queryKey: ["ia01"],
    queryFn: () => api.get("/ia01/asesi/all").then((res) => res.data?.data ?? res.data),
    refetchOnWindowFocus: false,
    refetchOnMount: false,
    staleTime: 60 * 60 * 1000,
  });

  // Select current user's submission
  const filteredData = ia01?.find(
      ...
      >
        ...
        >
          ...
          <div style={{ padding: "16px", overflowY: "auto" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                {buktiDescriptions.length > 0 ? (
                  buktiDescriptions.map((desc, i) => (
                    ...
                    >
                      <input type="checkbox" checked readOnly disabled style={{ transform: "scale(1.2)" }} />
                      {desc}
                    </label>
                  ))
                ) : (
            ...
            {/* Pernyataan (placeholder text matching earlier design) */}
            <div
              style={{
                border: "1.5px solid #ff8c42",
                borderRadius: "10px",
...
export default LihatApprovmentIa01;
