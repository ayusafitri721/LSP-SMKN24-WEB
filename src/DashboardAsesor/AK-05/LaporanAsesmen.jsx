...
const LaporanAsesmen = () => {
  ...
  const selectedAssesmenAsesi = assesmentAsesis.find((a) => a.assesi_id == nis);
  const selectedAssesment = assesments.find(
    (a) => a.id == selectedAssesmenAsesi?.assesment_id
  );
  console.log("assesmen", selectedAssesment);
  console.log("assesi ", selectedAssesmenAsesi);
  ...
  const handleSubmitAk05 = async () => {
    try {
      const body = {
        assesment_asesi_id: selectedAssesmenAsesi?.id,
        keputusan: "k", // atau ambil dari input jika ingin dinamis
        keterangan: formData.catatan,
        aspek_positif: formData.aspekNegatifPositif,
        aspek_negatif: formData.pencatatanPenolakan,
        penolakan_hasil: formData.pencatatanPenolakan,
        saran_perbaikan: formData.saranPerbaikan,
        ttd_asesor: "sudah",
      };
      const res = await api.post("/assesment/formak05", body);
      if (res.data.success) {
        setShowModal(true);
      }
    } catch (err) {
      alert("Gagal submit AK05");
      console.error(err);
    }
  };
  ...
  const validateForm = (action) => {
    ...
    const hasCompleteAsesi = formData.asesi.some(
      (asesi) =>
    ...
    if (!hasCompleteAsesi) {
      emptyFields.push(
        "Minimal satu data asesi lengkap (Nama, Rekomendasi, dan Keterangan)"
      );
    }

    // Validasi field feedback
    if (!formData.aspekNegatifPositif.trim())
      emptyFields.push("Aspek Negatif dan Positif dalam Asesmen");
    if (!formData.pencatatanPenolakan.trim())
      emptyFields.push("Pencatatan Penolakan Hasil Asesmen");
    if (!formData.saranPerbaikan.trim()) emptyFields.push("Saran perbaikan");

    // Validasi tanda tangan
    if (!formData.namaAsesorTtd.trim())
      emptyFields.push("Nama Asesor (Tanda Tangan)");
        ...
        <div style={formContainerStyle}>
            ...
            <table style={tableStyle} className="responsive-table">
              <tbody>
                ...
                <tr style={tableRowStyle}>
                  ...
                  <td
                    style={{ ...tableCellStyle, backgroundColor: "#f8f9fa" }}
                  ></td>
                  <td style={{ ...inputCellStyle, backgroundColor: "white" }}>
                    <input
                      style={inputStyle}
          ...
          <div style={{ margin: "20px 0" }}>
            ...
            <textarea
              ...
              onChange={(e) =>
                handleInputChange("aspekNegatifPositif", e.target.value)
              }
            />
          </div>
            ...
            >
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange("namaAsesorTtd", e.target.value)
                  }
                />
              </div>
              <div>
        ...
        {showModal && (
          <div style={modalOverlayStyle}>
            ...
            >
              ...
              >
                Anda menyetujui dokumen laporan asesmen ini dengan penilaian
