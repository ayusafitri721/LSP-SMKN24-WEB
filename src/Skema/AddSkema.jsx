...
import { SelectField } from "../components/FieldComponents";
...
function AddSkema({ onBack, onSave, onCancel }) {
  ...
  const { fetchJurusans } = useJurusan();
  ...
  const FILE_TYPES = [
    {
      key: "apl02",
      label: "APL.02 - Asesmen Mandiri",
      description: "Formulir asesmen mandiri peserta",
    },
    {
      key: "ak01",
      label: "AK.01 - Asesmen Kompetensi",
      description: "Panduan asesmen kompetensi",
    },
    {
      key: "ak04",
      label: "AK.04 - Checklist Observasi",
      description: "Lembar checklist observasi aktivitas",
    },
    {
      key: "ak02",
      label: "AK.02 - Jadwal Asesmen",
      description: "Jadwal pelaksanaan asesmen",
    },
    {
      key: "ak03",
      label: "AK.03 - Banding Asesmen",
      description: "Formulir banding asesmen",
    },
    {
      key: "ak05",
      label: "AK.05 - Umpan Balik",
      description: "Formulir umpan balik dan evaluasi",
    },
  ];
  ...
  const handleFileUpload = (fileType, file) => {
    if (
      ...
      setFormData((prev) => {
        ...
        // kalau fileType apl02 → set juga ke "file"
        if (fileType === "apl02") {
          newData.file = file;
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // pastikan dikirim sebagai integer (bukan object/string random)
      formDataToSend.append("jurusan_id", parseInt(formData.jurusan_id, 10));

      if (formData.file) {
        formDataToSend.append("file", formData.file); // backend expects "file"
      } else {
        throw new Error("File APL.02 harus diupload");
      }
      ...
      const res = await importFile(formDataToSend);
      ...
      setErrors({ global: err.message || "Gagal menyimpan data skema." });
    }
  };
