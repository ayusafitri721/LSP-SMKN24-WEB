...
import { submitFormIa01, fetchCsrfCookie, getAssesmentById, getFormIa01ByAssesi, getApl02ById } from '../../api/api';
...
const IA01 = () => {
  ...
  const [skemaUnits, setSkemaUnits] = useState([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      // Resolve skema_id for fetching full unit list
      const sid = chosen?.assesment?.skema_id || chosen?.skema_id || chosen?.schema_id || assesmentDetail?.skema_id;
      if (sid && sid !== resolvedSkemaId) setResolvedSkemaId(Number(sid));
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [JSON.stringify(userAssessments)]);

  // Fetch units list (kelompok pekerjaan) by skema_id
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Normalize to { kode_unit, judul_unit }
        const mapped = units.map(u => ({
          kode_unit: u.kode_unit || u.kode || u.code || '',
          judul_unit: u.judul_unit || u.judul || u.nama || u.name || ''
        }));
        setSkemaUnits(mapped);
      } catch (e) {
        setSkemaUnits([]);
      }
    })();
      ...
      <div style={contentCardStyle} className="content-card">
        ...
        <div style={responsiveInstructionSectionStyle}>
          ...
          <div style={kelompokPekerjaanStyle}>
            <div style={kelompokHeaderStyle}>Kelompok Pekerjaan</div>
            <ol style={kelompokListStyle}>
              {Array.isArray(skemaUnits) && skemaUnits.length > 0 ? (
                skemaUnits.map((u, idx) => (
                  <li key={`${u.kode_unit}-${idx}`} style={kelompokItemStyle}>
                    {u.kode_unit ? `${u.kode_unit} ` : ''}{u.judul_unit}
                  </li>
                ))
              ) : (
                <li style={kelompokItemStyle}>Belum ada unit untuk skema ini.</li>
              )}
            </ol>
          </div>
        </div>
