...
import Jadwal from "../DashboardAsesor/Jadwal"; // Tambahkan import ini
...
const DashboardAsesorRoutes = () => {
  return (
    <Routes>
      ...
      />
      <Route
        path="/lembar-jawaban/:nis"
        element={<LembarJawaban title="LEMBAR JAWABAN TERTULIS ESAI" />}
      />
      <Route path="/wawancara/:nis" element={<Wawancara title="WAWANCARA" />} />
      <Route
        path="/rekaman-asesmen/:nis"
        element={<RekamanAsesmen title="REKAMAN ASESMEN KOMPETENSI" />}
      />
      <Route
        path="/laporan-asesmen/:nis"
        element={<LaporanAsesmen title="LAPORAN ASESMEN" />}
      />
    </Routes>
  );
};
